<?php
    ...
    if (!isset($_SESSION['IdUsuario'])) {
        echo "ID de usuario no encontrado. Redirigiendo...";
    ...
    function pdf_descargar($conn, $userID) {
        ...
        if (!empty($curriculum)) {
            ...
            header('Content-Length: ' . strlen($curriculum)); // Establece el tamaño del contenido
            echo $curriculum;
            exit();
        } else {
            http_response_code(404); // Not Found
    ...
    // Consulta SQL para recuperar los datos de la imagen junto con el título y el GitHub
    $sql = "SELECT 
                pi.ID AS ImagenID,
                pi.Imagen AS ProyectoImagen,
                pi.Titulo AS Titulo,
                pi.GitHub AS GitHub
            FROM Usuario u
    ...
    $sql1 = "SELECT 
                ...
                e.Especialidad,
                p.Imagen AS PerfilImagen, 
                pr.ID AS ProyectoID, 
                pr.GitHub AS ProyectoGitHub,
    ...
    // Verifica si la imagen tiene datos
    if (!empty($imagenData)) {
    // Debug: Imprime la longitud de los datos binarios
    ...
    // Debug: Imprime mensaje si la imagen está vacía
    }
    }

    $sql2 = "SELECT 
                p.Gmail AS PerfilGmail,
                p.Telefono AS PerfilTelefono
            FROM Usuario u
            JOIN Perfil p ON u.IDPerfil = p.ID
            WHERE u.ID_Usuario = ?";
    ...
    function descargarCV(userID) {
        ...
        fetch('?action=descargarCV&userID=' + encodeURIComponent(userID))
            ...
            .then(blob => {
                ...
                a.href = url;
                a.download = 'cv_' + userID + '.pdf'; // Nombre del archivo
...
<body><!-- --------------------------------------------------------------------------- -->
    <nav id="desktop-nav">
        ...
        <div>
            <ul class="nav-links">
                ...
                <li><a href="listado.php">Ver perfiles</a></li>
            </ul>
        </div>
    </nav>
    ...
    <section id="profile">
        ...
        <div class="section__text">
            <p class="section_text_p1">¡Hola!, Soy</p>
            <?php echo '<h1 class="title">' . htmlspecialchars($perfilNombre) . '</h1>';?>
            <p class="section_text_p2"><?php echo htmlspecialchars($especialidad); ?></p>
    ...
    <section id="about">
        <p class="section_text_p1">Conozca más</p>
    ...
    <section id="experience">
        <p class="section_text_p1">Explora mi</p>
        ...
        <img src="../assets/images/portafolio/arrow.png" alt="Ícono de flecha" class="icon arrow" onclick="location.href='#projects'"/>
    </section>
    <section id="projects">
        <p class="section_text_p1">Explorar mis recientes</p>
